...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        $(function () { initSwipeBack();// 初始化滑动返回功�?
        ...
        function wxApi() {
            ...
            wx.config({
                debug: false, // 开启调试模�?调用的所有api的返回值会在客户端alert出来，若要查看传入的参数，可以在pc端打开，参数信息会通过log打出，仅在pc端时才会打印�?                appId: '<%=signModel.appId %>', // 必填，公众号的唯一标识
                timestamp: '<%=signModel.time %>', // 必填，生成签名的时间�?随便填写)
                nonceStr: '<%=signModel.randstr %>', // 必填，生成签名的随机�?随便填写)
                signature: '<%=signModel.signstr %>', // 必填，签名，见附�?
            ...
            wx.ready(function () {
                ...
                // config信息验证后会执行ready方法，所有接口调用都必须在config接口获得结果之后，config是一个客户端的异步操作，所以如果需要在页面加载时就调用相关接口，则须把相关接口放在ready函数中调用来确保正确执行。对于用户触发时才调用的接口，则可以直接调用，不需要放在ready函数中�?                isWxConfigReady = true;
            });
        ...
        function qrcode() {
            wx.scanQRCode({
                needResult: 1, // 默认�?，扫描结果由微信处理�?则直接返回扫描结果，
                scanType: ["qrCode", "barCode"], // 可以指定扫二维码还是一维码，默认二者都�?                success: function (res) {
                    var result = res.resultStr; // 当needResult �?1 时，扫码返回的结�?                    if (typeof (result) != "undefined") {
                    ...
                    <div id="divToolBar" name="divTooBar" class="ToolBar" style="width: 100%; text-align: left; padding-top: 10px; padding-bottom: 10px; background-color: aliceblue;"
                        ...
                        <table style="width: 100%;">
                            ...
                            <tr>
                                ...
                                <td class="ItemAlignLeft" style="padding-top: 2px;">
