...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        // 使用与第二个页面相同的加载方�?        var $load = function (loadFunc) {
            $(function () {
        ...
        $load(function () {
            ...
            // 绑定遮罩层点击关闭事�?            $('#popwindow_shade').on('click', function () {
                popClose();
            });
        ...
        function adjustPopupPosition($popup) {
            ...
            // 计算弹窗高度（不超过窗口高度�?5%�?            var maxPopupHeight = windowHeight - topPosition - 20; // 减去顶部和底部间�?            var popupHeight = Math.min(maxPopupHeight, 600);

            // 计算弹窗宽度（不超过窗口宽度�?5%�?            var popupWidth = Math.min(windowWidth * 0.95, 500);
            ...
            // 滚动到顶�?            $popup.find('.layui-layer-content').scrollTop(0);
        ...
        $(document).on('keydown', function (e) {
            if (e.keyCode === 27) { // ESC�?                if ($('#popwindow').is(':visible')) {
                    popClose();
                }
            }
        });
...
<body data-disable-pullrefresh="true">
    ...
    <!-- 等待图标 -->
    <img id="IMG_Waiting" src="Images/Processing.gif" alt="请稍候，处理�?.." style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999;" />
</body>
</html>