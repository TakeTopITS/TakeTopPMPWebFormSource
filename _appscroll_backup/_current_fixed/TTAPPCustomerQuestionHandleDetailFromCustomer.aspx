...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        var $load = function (loadFunc) {
            $(function () {
                initSwipeBack();// 初始化滑动返回功�?                if (typeof (Sys) != 'undefined') {
        ...
        $load(function () {
            //选择图片后压缩图�?            $("#AttachFile").change(function () {
                var _ua = window.navigator.userAgent;
                var _simpleFile = this.files[0];
                //判断是否为图�?                if (!/\/(?:jpeg|png|gif|png|bmp)/i.test(_simpleFile.type)) return;

                //插件exif.js获取ios图片的方向信�?                var _orientation;
                EXIF.getData(_simpleFile, function () {
                    _orientation = EXIF.getTag(this, 'Orientation');
                });

                //1.读取文件，通过FileReader，将图片文件转化为DataURL，即data:img/png;base64，开头的url，可以直接放在image.src�?
        ...
        function fixCalendarPosition() {
            // 找到弹窗中的日期输入�?            var dateInput = $('#popDetailWindow').find('input[id*="DLC_NextServiceTime"]');
            if (dateInput.length > 0) {
                // 找到日历容器
                var calendarContainer = $('.ajax__calendar_container');
                if (calendarContainer.length > 0) {
                    // 确保日历容器相对于弹窗定�?                    calendarContainer.css({
                        'position': 'absolute',
                        'z-index': '10001'
                    });
         ...
         * @return {String}       压缩后base64格式的图�?         */
        function compress(_img, _orientation) {
            //2.计算符合目标尺寸宽高值，若上传图片的宽高都大于目标图，对目标图等比压缩；如果有一边小于，对上传图片等比放大�?            var _goalWidth = 640,         //目标宽度
                _goalHeight = 480,         //目标高度
                _imgWidth = _img.naturalWidth,   //图片宽度
                _imgHeight = _img.naturalHeight,  //图片高度
                _tempWidth = _imgWidth,      //放大或缩小后的临时宽�?                _tempHeight = _imgHeight,     //放大或缩小后的临时宽�?                _r = 0;              //压缩�?
            ...
            //toDataURL方法，可以获取格式为"data:image/png;base64,***"的base64图片信息�?            var _data = _canvas.toDataURL('image/jpeg');
            return _data;
        }
        ...
        function upload() {
            $.ajax({
                ...
                //提交的数�?                data: { FileData: $("#imgData").val(), FileName: $("#AttachFile").val() },
                //返回数据的格�?                //在请求之前调用的函数
                beforeSend: function () {
                    $("#IMG_Waiting").show();
                },
        ...
        // 添加页面点击事件，点击弹窗外部关�?        document.addEventListener('click', function (e) {
            var popWindow = document.getElementById('popwindow');
            var popDetailWindow = document.getElementById('popDetailWindow');
            var popShade = document.getElementById('popwindow_shade');
        ...
        // 添加键盘事件，按ESC键关闭弹�?        document.addEventListener('keydown', function (e) {
            if (e.keyCode === 27) { // ESC�?                popClose();
            }
        });
    </script>
</head>
                                                ...
                                                <ContentTemplate>
                                                    <div style="display: flex; gap: 10px; align-items: center;">
                                                        ...
                                                        <div class="upload-button">
                                                            ...
                                                            <img id="IMG_Uploading" src="Images/Processing.gif" alt="请稍候，处理�?.." style="display: none;" />
                                                        </div>
                                                    </div>
                                                </ContentTemplate>
                                                <Triggers>
