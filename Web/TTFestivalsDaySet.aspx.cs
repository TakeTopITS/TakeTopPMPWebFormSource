...
public partial class TTFestivalsDaySet : System.Web.UI.Page
{
    ...
    protected void btn_copy_Click(object sender, EventArgs e)
    {
        if (int.Parse( this.ddl_year.SelectedValue) < int.Parse(this.ddl_copy.SelectedValue))
        {
            ...
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "Script2", "showAlertAtMouse('" +LanguageHandle.GetWord("ZZDiShiFuZhiChengGong")+"')", true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "Script1", "showAlertAtMouse('" +LanguageHandle.GetWord("ZZJingGaoFuZhiDaoDeNianFenBuNe")+"')", true);
        }
