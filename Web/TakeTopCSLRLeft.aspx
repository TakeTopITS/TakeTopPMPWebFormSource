...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        jQuery(document).ready(function () {
            ...
            setTimeout(function() {
                try {
                    ...
                    var parentFrame = window.parent.document.getElementById("TakeTopLRMDI");
                    
                    if (!parentFrame) {
                        console.log('TakeTopLRMDI not found');
                        return;
                    }
                    
                    var cols = parentFrame.cols;
                    ...
                    console.error('Error initializing left bar:', e);
                }
            }, 100);
        ...
        function toggleLeftBar() {
            ...
            setTimeout(function() {
                // 使用 ASP.NET AJAX 的 Sys.WebForms.PageRequestManager 来触发异步 PostBack
                if (typeof(Sys) !== 'undefined' && Sys.WebForms && Sys.WebForms.PageRequestManager.getInstance()) {
                    // 在 UpdatePanel 内，使用 __doPostBack 触发异步回发
                    __doPostBack('<%= BT_Extend.UniqueID %>', '');
                } else {
                    // 回退到普通 PostBack
                    __doPostBack('<%= BT_Extend.UniqueID %>', '');
                }
