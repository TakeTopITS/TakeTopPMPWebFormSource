...
public partial class TakeTopTimer : System.Web.UI.Page
{
    ...
    private void BackupSystemData()
    {
        ...
        try
        {
            ...
            {
                LogClass.WriteLogFile("Backup DB Begin:" + DateTime.Now.ToString());

                //自动备份系统数据库数据
                AutoBackupDBDataBySystem();

                LogClass.WriteLogFile("Backup DB End:" + DateTime.Now.ToString());
            }

            strDocBackupDataTime = GetDocBackupDataTime();
            if (strDocBackupDataTime == DateTime.Now.ToString("yyyyMMdd") & strCurrentTime == "2")
            {
                LogClass.WriteLogFile("Backup Doc Begin:" + DateTime.Now.ToString());

                //自动备份系统文档数据
                AutoBackupDocDataBySystem();

                LogClass.WriteLogFile("Backup Doc End:" + DateTime.Now.ToString());
            }
