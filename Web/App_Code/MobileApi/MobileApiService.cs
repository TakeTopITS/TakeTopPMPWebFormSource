...
public class MobileApiService
{
    ...
    protected bool OnLogin(string userName, string password, out string errorMsg)
    {
        ...
        strHQL = "from ProjectMember as projectMember where projectMember.UserCode = " + "'" + strUserCode + "'" + " and " + " projectMember.Password = " + "'" + strPassword + "'" + " and " + " rtrim(ltrim(projectMember.Status)) not in ( 'Stop','Resign')";
                ...
                if (strAllowDevice != "ALL")
                {
                    if (ShareClass.IsMobileDeviceCheckAgent())
                    {
                        if (strAllowDevice == "PC")
                        {
                            ...
                            //ClientScript.RegisterStartupScript(this.GetType(), "", "<script>showAlertAtMouse('警告，你不能用移动设备登录平台，请检查！');</script>");
                            return false;
                        }
                    }
                    else
                    {
                        if (strAllowDevice == "MOBILE")
                        {
                            ...
                            //ClientScript.RegisterStartupScript(this.GetType(), "", "<script>showAlertAtMouse('警告，你只能用移动设备登录平台，请检查！');</script>");
                            return false;
                        }
                    }
                }
        ...
        else
        {
            ...
            return false;
            //ClientScript.RegisterStartupScript(this.GetType(), "", "<script>showAlertAtMouse('用户代码或密码错误，或已被终止使用！');</script>");
        }
