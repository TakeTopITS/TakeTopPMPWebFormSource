...
public partial class TTAppCustomerService : System.Web.UI.Page
{
    string strUserCode;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        bool blVisible = TakeTopSecurity.TakeTopLicense.GetAuthobility(this.GetType().BaseType.Name + ".aspx", strUserCode);  //bool blVisible = TakeTopSecurity.TakeTopLicense.GetAuthobility(this.GetType().BaseType.Name + ".aspx","商机需求处理", strUserCode);
        if (blVisible == false)
        {
    ...
    protected void BT_AllQuestion_Click(object sender, EventArgs e)
    {
        ...
        strHQL = "from CustomerQuestion as customerQuestion where customerQuestion.OperatorCode In (Select customerQuestionHandleRecord.OperatorCode from CustomerQuestionHandleRecord as customerQuestionHandleRecord) order by customerQuestion.ID DESC";
        customerQuestionBLL = new CustomerQuestionBLL();
        lst = customerQuestionBLL.GetAllCustomerQuestions(strHQL);
