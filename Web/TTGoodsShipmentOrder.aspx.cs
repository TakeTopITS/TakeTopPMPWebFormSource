...
public partial class TTGoodsShipmentOrder : System.Web.UI.Page
{
    ...
    protected void AddDetail()
    {
        ...
        if (strFromGoodsID == "")
        {
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click11", "showAlertAtMouse('" + LanguageHandle.GetWord("ZZCWZYCKDZCJC") + "')", true);
            return;
        }

        if (CheckShipNumber(strFromGoodsID, deNumber) == false)
        {
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click22", "showAlertAtMouse('" + LanguageHandle.GetWord("ZZCWCKSLBNDYCLPKCSLJC") + "')", true);
            return;
        }
        ...
        if (strSourceType == "GoodsPJRecord")
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click33", "showAlertAtMouse('" +LanguageHandle.GetWord("ZZDiShiChuKuLiangChaoChuXuQiuL")+"')", true);
            }
        }
    ...
    protected void UpdateDetail()
    {
        ...
        if (strSourceType == "GoodsPJRecord")
        {
            if (!ShareClass.checkRequireNumberIsMoreHaveNumberForProjectRelatedItemNumber(strSourceID, "AleadyCheckOut", deNumber))
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click11", "showAlertAtMouse('" +LanguageHandle.GetWord("ZZDiShiChuKuLiangChaoChuXuQiuL")+"')", true);
            }
        }

        if (strFromGoodsID == "" | strGoodsCode == "" | strGoodsName == "" | strSpec == "" | strFromPosition == "")
        {
            ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click22", "showAlertAtMouse('" + LanguageHandle.GetWord("ZZYSRHYXDBNWKJC") + "')", true);
        }
        else
        {
            GoodsShipmentDetailBLL goodsShipmentDetailBLL = new GoodsShipmentDetailBLL();
