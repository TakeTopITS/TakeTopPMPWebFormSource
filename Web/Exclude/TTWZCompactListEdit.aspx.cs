    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(strProjectCode))
            {
                ...
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('请选择工程项目！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                return;
            }
            if (string.IsNullOrEmpty(strSupplierCode))
            {
                ...
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('请选择供应商！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                return;
            }
            if (string.IsNullOrEmpty(strCompactName))
            {
                ...
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('合同名称不能为空，请补充！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                return;
            }
            if (string.IsNullOrEmpty(strControlMoney))
            {
                ...
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('价格监审不能为空，请补充！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                return;
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('未找到采购工程师对应的需方记录！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                    return;
                }
            }
        ...
        if (!string.IsNullOrEmpty(strCompactCode))
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strCompactText))
                {
                    ...
                    if (fi.Exists)
                    {
                        ClientScript.RegisterStartupScript(this.GetType(), "", "<script>showAlertAtMouse('存在同名文件，上传失败，请改名后再上传！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');</script>");
                        return;
                    }
                    ...
                    //重新加载报价文件列表
                    ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('上传合同文本成功！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('请选择要上传的文件！');ControlStatusChange('" + strProgress + "','" + strIsMark + "');", true);
                    return;
                }
            }
            catch (Exception ex) { }
