...
SELECT
    ...
    //增加操作导航模块
    public static void InsertOperateNavigationModule()
    {
        string strHQL;
        string strOperationSql;

        strHQL = "Select * From T_ProModuleLevel Where ModuleName = 'OperateNavigation'";
        DataSet ds = ShareClass.GetDataSetFromSql(strHQL, "T_ProModuleLevel");
        if (ds.Tables[0].Rows.Count > 0)
        {
            LogClass.WriteLogFile("OperateNavigation module already exists.");
            return;
        }
        else
        {
            try
            {
                ...
                strOperationSql = string.Format(@"   Delete From T_ProModuleLevel Where ModuleName = 'OperateNavigation';
                    ...
                    Insert Into T_ProModule(UserCode, ModuleName, ModuleType, UserType, ModuleDefinition, DiyFlow, Visible)
                        ...
                        values('SAMPLE', 'OperateNavigation', 'SYSTEM', 'INNER', '', 'YES', 'YES');
                    Insert Into T_ProModule(UserCode, ModuleName, ModuleType, UserType, ModuleDefinition, DiyFlow, Visible)

                    values('SAMPLE', 'OperateNavigation', 'SYSTEM', 'OUTER', '', 'YES', 'YES');");

                ShareClass.RunSqlCommand(strOperationSql);

                LogClass.WriteLogFile(strOperationSql);
            }
            catch (Exception err)
            {
                LogClass.WriteLogFile("Error page: " + "\n" + err.Message.ToString() + "\n" + err.StackTrace);
    ...
    public static void UpdateModuleFlowDefinition()
    {
        ...
        try
        {
            ...
            if ("ECMP,DEMO".IndexOf(strSystemProductType) > -1)
            {
                ...
                SaveModuleFlowDefinition("OperateNavigation", strMFXML, 3, "INNER");
            }
            ...
            SaveModuleFlowDefinition("OperateNavigation", strMFXML, 3, "OUTER");
        }
        catch (Exception err)
        {
            LogClass.WriteLogFile("Error page: " + "\n" + err.Message.ToString() + "\n" + err.StackTrace);
