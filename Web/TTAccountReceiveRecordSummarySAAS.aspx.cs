...
public partial class TTAccountReceiveRecordSummarySAAS : System.Web.UI.Page
{
    ...
    protected void BT_Query_Click(object sender, EventArgs e)
    {
        ...
        //strHQL = @"Select (COALESCE(A.ReceiverTime,B.OutOfPocketTime) ||('(" + LanguageHandle.GetWord("ShouKuan") + "," + LanguageHandle.GetWord("FuKuan") + "," + LanguageHandle.GetWord("Banlance") + ")')) as XName,";

        strHQL = @"Select (COALESCE(A.ReceiverTime,B.OutOfPocketTime) ||('(Pay,Receive,Banlance)')) as XName,";
        strHQL += @"COALESCE(A.ReceiverMoney,0) AS HNumber,COALESCE(B.PayMoney,0) AS YNumber,
                 COALESCE(A.ReceiverMoney,0)- COALESCE(B.PayMoney,0) AS ZNumber 
    ...
    protected void LoadConstractReceivablesRecord(string strDepartString)
    {
        ...
        strHQL = @"Select (COALESCE(A.ReceiverTime,B.OutOfPocketTime) ||('(Receive,Pay,Banlance)')) as XName,";
        strHQL += @"COALESCE(A.ReceiverMoney,0) AS HNumber,COALESCE(B.PayMoney,0) AS YNumber,
                 COALESCE(A.ReceiverMoney,0)- COALESCE(B.PayMoney,0) AS ZNumber 
                 From (SELECT to_char(ReceiverTime,'yyyymm') ReceiverTime,SUM(COALESCE(ReceiverAccount * ExchangeRate,0)) ReceiverMoney 
                 FROM T_ConstractReceivablesRecord";
