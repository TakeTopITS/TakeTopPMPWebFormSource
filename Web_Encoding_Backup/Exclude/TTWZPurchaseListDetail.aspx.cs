...
public partial class TTWZPurchaseListDetail : System.Web.UI.Page
{
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('文件名称不能为空，请补充！');", true);
                return;
            }
            if (string.IsNullOrEmpty(strProjectCode))
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('项目编码不能为空，请补充！');", true);
                return;
            }
            if (string.IsNullOrEmpty(strPurchaseEngineer))
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('采购工程师不能为空，请补充！');", true);
                return;
            }
            if (string.IsNullOrEmpty(strDelegateAgent))
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('委托代理人不能为空，请补充！');", true);
                return;
            }
            if (string.IsNullOrEmpty(strPurchaseManager))
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('采购经理不能为空，请补充！');", true);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(HF_PurchaseCode.Value))
            {
                ...
                if (listPurchase != null && listPurchase.Count > 0)
                {
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('采购文件进度不为录入，不允许修改！');", true);
                        return;
                    }
