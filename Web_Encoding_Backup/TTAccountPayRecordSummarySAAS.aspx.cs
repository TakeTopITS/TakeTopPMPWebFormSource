...
public partial class TTAccountPayRecordSummarySAAS : System.Web.UI.Page
{
    ...
    protected void BT_Query_Click(object sender, EventArgs e)
    {
        ...
        //strHQL = @"Select (COALESCE(A.OutOfPocketTime, B.ReceiverTime) ||('(" + LanguageHandle.GetWord("FuKuan") + "," + LanguageHandle.GetWord("ShouKuan") + "," + LanguageHandle.GetWord("Banlance") + ")')) as XName,";

        strHQL = @"Select (COALESCE(A.OutOfPocketTime, B.ReceiverTime) ||('(Pay,Receive,Banlance)')) as XName,";
        strHQL += @"COALESCE(A.PayMoney, 0) AS HNumber, COALESCE(B.ReceiverMoney, 0) AS YNumber,
            COALESCE(A.PayMoney, 0)-COALESCE(B.ReceiverMoney, 0) AS ZNumber
