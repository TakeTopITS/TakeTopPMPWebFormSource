...
public partial class TTWorkPlanGanttForProject : System.Web.UI.Page
{
    protected int pid = 1;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (strLangCode != "zh-CN" & strLangCode != "zh-tw")
        {
            if (strBusinessType != "COMPARE")
            {
                Response.Redirect("TTWorkPlanGanttForProjectForEnglish.aspx?pid=" + pid.ToString());
            }
            else
            {
                Response.Redirect("TTWorkPlanGanttForProjectForEnglish.aspx?pid=" + pid.ToString() + "&BusinessType=" + strBusinessType);
            }
        }
        if (strLangCode == "zh-tw")
        {
            if (strBusinessType != "COMPARE")
            {
                Response.Redirect("TTWorkPlanGanttForProjectForTraditional.aspx?pid=" + pid.ToString());
            }
            else
            {
                Response.Redirect("TTWorkPlanGanttForProjectForTraditional.aspx?pid=" + pid.ToString() + "&BusinessType=" + strBusinessType);
            }
        }
