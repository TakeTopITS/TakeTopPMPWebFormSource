...
var jqueryDocumentReady = function() {
    ...
    ////给右边栏AI窗口查询提示框赋值示例代码
    //setAIWindowPromptText('提示词')

    ////调用右边栏AI窗口取值示例代码
    //document.getElementById('_0_1_my:TxtQueryReturnValue').innerText = getAIWindowReturnText();

    ////控件上加事件，双击控件时调用右边栏AI窗口取值示例代码
    //document.getElementById('_0_1_my:TxtQueryReturnValue').addEventListener('dblclick', function() {
    //   document.getElementById('_0_1_my:TxtQueryReturnValue').innerText = getAIWindowReturnText();
    //   ////通过代码改变的控件内容，必须调用此方法才能保存
    //   TakeTopInfoPath_OnControlChange(null, document.getElementById('_0_1_my:TxtQueryReturnValue'));
    // });

    ////直接调用AI API示例代码
    //var result = await callAIByAIAPI('提示词');
...
//// 前端调用AI API使用示例
 async function callAIByAIAPI(text) {
    try {
        const result = await callAIWithFetch(text);
...
";
    ...
    protected void BT_DeleteByID_Click(object sender, EventArgs e)
    {
        ...
        if (Repeater1.Items.Count > 1)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this.UpdatePanel1, this.GetType(), "click", "showAlertAtMouse('" +LanguageHandle.GetWord("ZZDiShiQingXianShuaZeYaoShanCh")+"')", true);
            }
        }
    }
